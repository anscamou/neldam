...
export class PaymentService {
  ...
  protected convertDateFromClient(payment: IPayment): IPayment {
    const copy: IPayment = Object.assign({}, payment, {
      placeDate: payment.placeDate && payment.placeDate.isValid() ? payment.placeDate.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((payment: IPayment) => {
        payment.placeDate = payment.placeDate ? moment(payment.placeDate) : undefined;
      });
    }
    return res;
  }
