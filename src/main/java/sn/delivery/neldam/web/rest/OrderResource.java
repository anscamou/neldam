...
/**
 * REST controller for managing {@link sn.delivery.neldam.domain.Order}.
 */
@RestController
@RequestMapping("/api")
public class OrderResource {
    ...
    public ResponseEntity<List<OrderDTO>> getAllOrders(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("order-is-null".equals(filter)) {
            log.debug("REST request to get all Orders where order is null");
            return new ResponseEntity<>(orderService.findAllWhereOrderIsNull(),
                    HttpStatus.OK);
        }
